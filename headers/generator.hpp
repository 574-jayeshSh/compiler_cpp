...
class Generator{
    ...
    unordered_map <enum NODE_TYPE , string> relationalReferences
    {
    	  {NODE_REL_EQUALS            , "je"},
    ...
    string generateEXIT(AST_NODE * STATEMENT)
    {
        stringstream exitBuffer;
        ...
        switch(STATEMENT->CHILD->TYPE)
        {
            ...
            case NODE_VARIABLE : 
            {
                ...
                // exitBuffer << "mov eax , dword [rbp - " << to_string(offset) << "]\n";
                // exitBuffer << "mov rdi , rax\n";
                // exitBuffer << "mov rax , 60\n";
                // exitBuffer << "syscall\n\n";
                
                cout << "[~] WARNING : Varible return types are as of now not supported , exiting with code 0" << endl;
                // exitBuffer << "mov rax , 60\n";
    ...
    int stringVariableExists(string * TEST_STRING)
    {
    ...
    vector<int> convertToRequiredDimensions (vector<int> dimensions)
    {
        vector<int> newDimensions(dimensions.size());
        int writeValue = 1;
    ...
    int variableReferenceExists(string * TEST_STRING)
    {
        int counter = 0;
        for (int i = variableVector.size() - 1 ; i >= 0  ; i--)
        {
            string currentValue = variableVector[i];
...
// this is the start of the helper functions
    int functionExists(string * FUNCTION_NAME)
    {
    ...
    int arrayExists(string * ARRAY_NAME)
    {
    ...
    void arrayCheck(AST_NODE * ARRAY_NODE)
    {
		...
		{
			cout << "[!] Error , the given array does not exists : " << *ARRAY_NODE->VALUE << endl;
			exit(1);
		}
		if (ARRAY_NODE->SUB_VALUES.size() != arrayTable[*ARRAY_NODE->VALUE].size())
		{
			cout << "[!] Dimensional error while accessing the array : " << *ARRAY_NODE->VALUE;
		       exit(1);	
		}
    }
    
    void createNewInteger(string VARIABLE_NAME , int * elemOffset)
    {
    ...
    int handleStringData(string * STRING_DATA)
    {
    ...
    int getIntegerVariableOffset(string * VARIABLE_NAME)
    {
        int elemOffset = variableReferenceExists(VARIABLE_NAME);
        if (elemOffset == -1)
        {
            cout << "[!] SYNTAX ERROR : Undefined integer variable : " << *VARIABLE_NAME << endl;
            exit(1);
        }
        return (offsetCounter - elemOffset) * 4;
    }

    void checkUniqueID(string * VARIABLE_NAME , enum NODE_TYPE ACCEPTABLE_TYPE = NODE_ROOT) 
    {
        ...
        if (sData != -1 && ACCEPTABLE_TYPE != NODE_STRING)
            {cout << "[!] Error : Cannot redeclare the following as string : " << *VARIABLE_NAME; exit(1);}
        if (vData != -1 && ACCEPTABLE_TYPE != NODE_INT)
            {cout << "[!] Error : Cannot redeclare the following integer variable : " << *VARIABLE_NAME; exit(1);}
        if (fData != -1 && ACCEPTABLE_TYPE != NODE_FUNCTION)
            {cout << "[!] Error : Cannot redeclare the following function : " << *VARIABLE_NAME; exit(1);}
        if (aData != -1 && ACCEPTABLE_TYPE != NODE_ARRAY)
            {cout << "[!] Error : Cannot redeclare the following array : " << *VARIABLE_NAME; exit(1);}
    }

    void inScopeGeneration(vector <AST_NODE *> STATEMENTS)
    {
	    ...
	    switch(CHILD_STATEMENT->TYPE)
            {
                ...
                default : 
                {
                    cout << "[!] ERROR : FOUND : "  << nodeToString(CHILD_STATEMENT->TYPE); 
                    cout << " IN INDENT SCOPE OF THE PROGRAM: " ;
                    exit(0); 
                    break;
                }
            }
    ...
    void functionalInScopeGeneration(vector <AST_NODE *> STATEMENTS)
    {
        for (AST_NODE * CHILD_STATEMENT : STATEMENTS)
       {
	    switch(CHILD_STATEMENT->TYPE)
            {
                ...
                {
                    cout << "[!] ERROR : FOUND : "  << nodeToString(CHILD_STATEMENT->TYPE); 
                    cout << " IN FUNCTION SCOPE OF THE PROGRAM: " ;
                    exit(0); 
                    break;
                }
            }
    ...
    void calculateArrayOffset(AST_NODE * ARRAY_NODE) // THE OFFSET VALUE WOULD BE AT THE TOP OF THE STACK
    {
        ...
        vector <int> requiredDimensions = arrayTable[*ARRAY_NODE->VALUE];
		...
		for (int dimensionalCounter = 0 ; dimensionalCounter < ARRAY_NODE->SUB_VALUES.size() ; dimensionalCounter++)
		{
			...
			switch(INDEX_NODE->TYPE) // this value would go in the eax register
			{
			    ...
			    case NODE_VARIABLE :
			    {
				    ...
				    if (indexOffset == -1)
				    {
					    cout << "[!] Undefined Variable " << *INDEX_NODE->VALUE;
					    exit(1);
				    }
				    indexOffset = (offsetCounter - indexOffset) * 4;
			        sectionText << "mov eax , dword [rbp - " << to_string(indexOffset) << "]\n";
				    break;
			    }
			}

			sectionText << "mov ebx , " << to_string(requiredDimensions[dimensionalCounter]) << "\n";
...
// this is the end of the helper functions
    ...
    void sectionDataDefine(string * NEW_ENTRY , int referenceNumber)
    {
        sectionData << "@Sref" + to_string(referenceNumber);
        sectionData << " :\n";
        sectionData << "db \"" + *NEW_ENTRY + "\"\n";
        sectionData << "@Sref" + to_string(referenceNumber);
        sectionData << "_L : equ $-@Sref" + to_string(referenceNumber);
        sectionData << "\n\n"; 
    ...
    void generateGET(AST_NODE * STATEMENT) // OP0 DONE 
    {
        ...
        if (stringVariableExists(VAR_ID->VALUE) != -1) {cout << "[!] SYNTAX ERROR : cannot use string variable in INPUT : " << *VAR_ID->VALUE; exit(1);}
        if (functionExists(VAR_ID->VALUE) != -1) {cout << "Cannot name the variable as same as previously defined function : " << *VAR_ID->VALUE; exit(1);}
        ...
        switch (VAR_ID->TYPE)
        {
            case NODE_VARIABLE:
            {
                ...
                sectionText << "\nmov dword [rbp - " << to_string(offset) << " ] , eax\n";
                break;
            }
        ...
        switch (STATEMENT->CHILD->TYPE)
        {
            case NODE_VARIABLE: 
            {
                ...
                if (elemOffset == -1)
                {
                    ...
                    {
                        cout << "[!] SYNTAX ERROR : undefined varialble : " << *STATEMENT->CHILD->VALUE;
                        exit(1);
                    }
                    sectionText << "mov rax , 1\n";
                    sectionText << "mov rdi , 1\n";
                    sectionText << "mov rsi , @Sref" + to_string(stringReferenceNumber) + "\n";
                    sectionText << "mov rdx , @Sref" + to_string(stringReferenceNumber);
                    sectionText << "_L \nsyscall \n\n";
                ...
                else 
                {
                    ...
                    sectionText << "mov eax , dword [rbp - " << to_string(offset);
                    sectionText << "]\ncall _printINTEGER\n\n";
                }
                break;
            }
            ...
            case NODE_STRING :
            {
                ...
                string displayLoopSegment = "_DISPLAY_LOOP_" + to_string(displayLoopCounter);
                string displayLoopSegmentLHS = "_DISPLAY_LOOP_" + to_string(displayLoopCounter) + "_LHS";                 
                int lhsOffset;
                int limitOffset;  
                ...
                if (STATEMENT->LIMIT)
                {
                    ...
                    switch (STATEMENT->LIMIT->TYPE)
                    {
                        ...
                        case NODE_INT :
                        {
                            ...
                            sectionText << "mov dword [ rbp - " << to_string(limitOffset) << " ] , " << *STATEMENT->LIMIT->VALUE << "\n";
                            break;
                        }
                        case NODE_MATH:
                        {  
                            ...
                            createNewInteger(displayLoopSegment , &limitOffset);
                            sectionText << "mov dword [ rbp - " << to_string(limitOffset) << " ] , eax\n";
                            break;
                        }
                    }
                    createNewInteger(displayLoopSegmentLHS , &lhsOffset);
                    sectionText << "mov dword [ rbp - " << to_string(lhsOffset) << " ] , 1\n";
                    sectionText << displayLoopSegment << ":\n";
                    sectionText << "mov eax , dword [ rbp - " << to_string(lhsOffset) << " ] \n";
                    sectionText << "mov ebx , dword [ rbp - " << to_string(limitOffset) << " ] \n";
                ...
                sectionText << "mov rax , 1\n";
                sectionText << "mov rdi , 1\n";
                sectionText << "mov rsi , @Sref" + to_string(referenceNumber) + "\n";
                sectionText << "mov rdx , @Sref" + to_string(referenceNumber);
                sectionText << "_L \nsyscall \n\n";

                if (STATEMENT->LIMIT)
                {
                    sectionText << "add dword [ rbp - " << to_string(lhsOffset) << " ] , 1\n";
    ...
    {
        string stringLeft = "@_LOOP_" + to_string(loopBranchCounter) + "_LEFT";
        string stringRight = "@_LOOP_" + to_string(loopBranchCounter) + "_RIGHT";
        ...
        switch (STATEMENT->CHILD->TYPE)
        {
            ...
            case NODE_INT :
            {
                ...
                sectionText << "mov dword [ rbp - " << to_string(offsetLeft) << " ] , " << *STATEMENT->CHILD->VALUE << "\n";
                break;
            }
            case NODE_MATH:
            {
                generateMATH(STATEMENT->CHILD->SUB_STATEMENTS);
                createNewInteger(stringLeft , &offsetLeft);
                sectionText << "mov dword [ rbp - " << to_string(offsetLeft) << " ] , eax\n";
                break;
            }
        }
        switch (STATEMENT->LIMIT->TYPE)
        {
            ...
            case NODE_INT :
            {
                ...
                sectionText << "mov dword [ rbp - " << to_string(offsetRight) << " ] , " << *STATEMENT->LIMIT->VALUE << "\n";
                break;
            }
            case NODE_MATH:
            {
                generateMATH(STATEMENT->LIMIT->SUB_STATEMENTS);
                createNewInteger(stringRight , &offsetRight);
                sectionText << "mov dword [ rbp - " << to_string(offsetRight) << " ] , eax\n";
                break;
            }
        }
        ...
        sectionText << "jmp _LOOP_" << to_string(loopBranchCounterCopy) << "_ENTRY \n";
        sectionText << "_LOOP_" << to_string(loopBranchCounterCopy) << "_ENTRY: \n";
        
        if (supplementOffset) 
        {
        sectionText << "mov eax , dword [ rbp - " << to_string(offsetLeft) << " ]\n";
        sectionText << "mov dword [rbp - " << to_string(supplementOffset) << "] , eax\n";
        }

        sectionText << "mov eax , dword [ rbp - " << to_string(offsetLeft) << " ] \n";
        sectionText << "mov ebx , dword [ rbp - " << to_string(offsetRight) << " ] \n";
        sectionText << "cmp eax , ebx\n";
        sectionText << "jg _LOOP_" << to_string(loopBranchCounterCopy) << "_EXIT \n";

        inScopeGeneration(STATEMENT->SUB_STATEMENTS);

        sectionText << "add dword [ rbp - " << to_string(offsetLeft) << " ] , 1\n";
        sectionText << "jmp _LOOP_" << to_string(loopBranchCounterCopy) << "_ENTRY \n";
        sectionText << "_LOOP_" << to_string(loopBranchCounterCopy) << "_EXIT: \n";
    }
    ...
    void generateTILL (AST_NODE * STATEMENT) // OP0
    {
        ...
        tillCounter++;
        sectionText << "_TILL_" << to_string(tillCounterCopy) << "_ENTRY:\n";

        AST_NODE * BUFFERPOINTER;
        string registerPlaceHolder;
        sectionText << "\n";
        ...
        for (int itr = 0 ; itr <= 2 ; itr++)
        {
            ...
            switch (BUFFERPOINTER->TYPE)
            {
                ...
                case NODE_VARIABLE : 
                {
                    if (stringVariableExists(BUFFERPOINTER->VALUE) != -1) {cout << "[!] SYNTAX ERROR : Cannot use the following string variable inside TILL : " << *BUFFERPOINTER->VALUE; exit(1);}
        ...
        sectionText << relationalReferences[BUFFERPOINTER->TYPE] << " _TILL_" << to_string(tillCounterCopy) <<"_MAIN\n";
        sectionText << "jmp _TILL_" << to_string(tillCounterCopy) << "_EXIT\n\n";
        sectionText << "_TILL_" << to_string(tillCounterCopy) <<"_MAIN:\n";
        ...
        sectionText << "jmp _TILL_"  << to_string(tillCounterCopy) << "_ENTRY\n"; 
        sectionText << "_TILL_" << to_string(tillCounterCopy) << "_EXIT:\n";
    ...
    void generateIF(AST_NODE * STATEMENT) // OP0
    {
        ...
        for (int itr = 0 ; itr <= 2 ; itr++)
        {
            ...
            switch (BUFFERPOINTER->TYPE)
            {
                ...
                if (stringVariableExists(BUFFERPOINTER->VALUE) != -1) {cout << "[!] SYNTAX ERROR : cannot use string variable in if : " << *BUFFERPOINTER->VALUE; exit(1);}
        ...
        sectionText << relationalReferences[BUFFERPOINTER->TYPE] << " _BRANCH_" << to_string(branchCounter) <<"_IF\n";
        sectionText << "jmp _BRANCH_" << to_string(branchCounter) <<"_ELSE\n\n";
        sectionText << "_BRANCH_" << to_string(branchCounter) <<"_IF:\n";
       ...
       sectionText << "jmp _BRANCH_" << to_string(branchCounterCopy) <<"_EXIT\n";     
       sectionText << "_BRANCH_" << to_string(branchCounterCopy) <<"_ELSE:\n";
       
       if (STATEMENT->SUPPLEMENT)
         generateELSE(STATEMENT->SUPPLEMENT);
       
       sectionText << "jmp _BRANCH_" << to_string(branchCounterCopy) <<"_EXIT\n\n";     
       sectionText << "_BRANCH_" << to_string(branchCounterCopy) <<"_EXIT:\n";
    ...
    void generateFUNCTION (AST_NODE * STATEMENT) // OP0 DONE
    {
       ...
       for (AST_NODE * ARGUMENT : STATEMENT->SUB_VALUES)
       {
	       ...
	       // going over the variable Vector in the opposite direction would take care of it
            ...
            sectionText << "mov dword [rbp - "<< to_string(offset * 4) << "] , eax \n";
	        argumentNumber++;
       }

       functionalInScopeGeneration(STATEMENT->SUB_STATEMENTS); 

    //    sectionText << "add rsp , " << to_string((offsetCounter - initOffsetCounter)*4) << "\n";
    ...
    //    sectionText << "ret ; this is the final return and hsould be at the last \n\n";
       ...
       functionDefinitions << "add rsp , " << to_string((offsetCounter - initOffsetCounter)*4) << "\n";
    ...
    void generateCALL (AST_NODE * STATEMENT) // THE RETURN VALUE WOULD BE IN THE RAX REGISTER
    {
        ...
        {
            cout << "[!] Error : Undefined Function : " << *STATEMENT->CHILD->VALUE;
            exit(1);
        }
	    if (STATEMENT->SUB_VALUES.size() != functionTable[*STATEMENT->CHILD->VALUE])
	    {
		    cout << "[!] Error : The function : " << *STATEMENT->CHILD->VALUE << " expected " << functionTable[*STATEMENT->CHILD->VALUE] << " arguments , but the number of arguments provided in the call were: " << STATEMENT->SUB_VALUES.size() << endl;
		    exit(1);
	    }
        int registerHolderCounter = 0;
        if (STATEMENT->SUB_VALUES.size() > 6)
        {
			cout << "Sorry , no more than 6 arguments are allowed in the current version of pilot..." << endl;
			exit(1);
		}
    ...
    void generateRETURN (AST_NODE * STATEMENT)
    {
        ...
        {
            cout << "[!] Syntax Error : Cannot have return statement outside of functional block";
            exit(0);
        }
        switch(STATEMENT->CHILD->TYPE)
        {
    ...
    void generateMATH(vector <AST_NODE * >& operations ) // THE CALCULATED VALUE IS STORED IN THE RAX REGISTER
    {
        ...
        for (AST_NODE * CURRENT : postfixOperations)
        {
            switch (CURRENT->TYPE)
            {
                ...
                case NODE_VARIABLE : 
                {
                    ...
                    sectionText << "mov eax , dword [rbp - " + to_string(offset_RHS);
                    sectionText << "]\npush rax\n";
                    break;
                }
    ...
    void generateARRAY (AST_NODE * STATEMENT) // OP0 DONE
    {
        ...
        for (AST_NODE * TEMP : STATEMENT->SUB_VALUES)
        {
	        ...
	        {
		        cout << "The current support is only for integer dimensions , sorry..." << endl;
		        exit(1);
    	    }
	        dimensions.push_back(stoi(*TEMP->VALUE));
            arraySpace = arraySpace == 0 ? arraySpace + stoi(*TEMP->VALUE) : arraySpace * stoi(*TEMP->VALUE); 
        }
        sectionBss << *STATEMENT->VALUE << " : resd "<< to_string(arraySpace) << "\n\n";
        ...
        for (AST_NODE * CURRENT_STATEMENT : STATEMENT->SUB_STATEMENTS)
        {
	        switch (CURRENT_STATEMENT->TYPE)
	        {
	            ...
	            case NODE_VARIABLE: 
            	{
                	...
                	sectionText << "\nmov eax , dword [rbp - " + to_string(offset_RHS);
             		sectionText << "]\nmov [ebx] , eax\n";
                	break;
            	}	
	       }
    ...
    void generateARRAY_VARIABLE (AST_NODE * ARRAY_ID)
    {
        ...
        switch (ARRAY_ID->CHILD->TYPE) 
        {
            ...
            default : {
                cout << "[!] Generation Error : Error while switch the following child of ARRAY_VARIABLE : " << nodeToString(ARRAY_ID->CHILD->TYPE) << endl;
                exit(1);
            }
        }
    }
    ...
    void generateVARIABLE(AST_NODE * VAR_ID)
    {
	    ...
	    {
		    cout << "[!] Cannot same the variable same as already defined function : " << *VAR_ID->VALUE;
		    exit(1);
	    }
        if (VAR_ID->CHILD->TYPE != NODE_STRING)  // cross linkage error     
        {
            ...
            {
                cout << "[!] SYNTAX ERROR : Cannot cross link a string and an integer variable" << *VAR_ID->VALUE; 
                exit(1);
            }
    	}
        ...
        switch (VAR_ID->CHILD->TYPE) // MAJOR THING : ADD SUPPORT FOR RHS ARRAY VALUES 
        {
            case NODE_INT:
            {
                sectionText << "mov dword [rbp - " << to_string(offset);
            ...
            case NODE_VARIABLE: 
            {
                ...
                sectionText << "\nmov eax , dword [rbp - " + to_string(offset_RHS);
                sectionText << "]\nmov dword [rbp - " + to_string(offset);
                sectionText << "] , eax \n\n";
                break;
            }
            case NODE_ARRAY_VARIABLE:
            ...
            case NODE_STRING:
            {
                ...
                if (elemOffset != -1)
                {
                    cout << "[!] SYNTAX ERROR : Cannot Cross link a string and an integer variable"; 
                    exit(1);
                }
            ...
            default : 
            {
                cout << "[!] Generation Error : unidentidfied linkage to variable assignment : " << nodeToString(VAR_ID->CHILD->TYPE) << endl;
                exit(1);
            }
        }
    }
    ...
    void generate()
    {
        ...
        for (AST_NODE * CURRENT : AST_ROOT->SUB_STATEMENTS)
        {
            ...
            switch(CURRENT->TYPE)
            {
                ...
                default : 
                {
                    cout << "[!] ERROR : FOUND : "  << nodeToString(CURRENT->TYPE); 
                    cout << " IN GLOBAL SCOPE OF THE PROGRAM" ;
                    exit(0); 
                    break;
                }
            }
        ...
        stackReset << "\nadd rsp , " + to_string(offsetCounter * 4);
        stackReset << "\nmov rsp , rbp\n";
        stackReset << "pop rbp\n\n";
        writeSections();
    }
    ...
    stringstream functionDefinitions;
    stringstream stackReset;
    ...
    unordered_map <string , int> functionTable;
    unordered_map <string , vector<int>> arrayTable; // string : name , vector<int> : dimensions
    vector <string> registerArgumentHolders = {"rdi" , "rsi" , "rdx" , "rcx" , "r8" , "r9"};
